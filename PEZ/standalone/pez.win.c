...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE ignoreMe0, LPSTR ignoreMe1, INT ignoreMe2)
{
	...
	const PezConfig pCFG = PezGetConfig();

    SetRect(&rect, 0, 0, pCFG.Width, pCFG.Height);
	...
	SetWindowTextA(hWnd, pCFG.Title);
    ...
    if (!pCFG.VerticalSync)
    {
        wglSwapIntervalEXT(0);
    }

	PezInitialize(pCFG.Width, pCFG.Height);
